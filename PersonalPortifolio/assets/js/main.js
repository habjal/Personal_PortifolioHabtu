...
function initCounter() {
  ...
  const wowNumber = new WOW({
    ...
    callback: function (box) {
      function getAnimationDelayInMs(element) {
        const delay = getComputedStyle(element).animationDelay;
        if (!delay) return 0;
        return delay.includes("s")
          ? parseFloat(delay) * 1000
          : parseInt(delay);
      }
      const animationDelay = getAnimationDelayInMs(box) + 600;
...
function initSplittingElement() {
  ...
  elements.forEach(el => {
    if (el.dataset.splittingDuration) {
      el.style.setProperty('--transition-duration', `${el.dataset.splittingDuration}`);
    }
    if (el.dataset.splittingDelay) {
      el.style.setProperty('--transition-delay', `${el.dataset.splittingDelay}`);
    }
    // if (el.dataset.staDelay) {
    //   el.style.setProperty('--splitting-transition-delay', `${el.dataset.staDelay}ms`);
    // }
  });
  Splitting();
}
